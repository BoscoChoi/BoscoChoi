# Hi! I'm Bosco

[![LinkedIn Badge](https://img.shields.io/badge/-LinkedIn-blue?style=flat-square&logo=Linkedin&logoColor=white&link=YOUR_LINKEDIN_URL)](https://www.linkedin.com/in/bosco-choi/)


👋 Hi, I’m an urban planning student at UCL with an interest in data-driven approaches to urban planning and design.

I’m currently building Python-based projects to explore urban issues, combining spatial data, policy analysis, and quantitative methods. Always learning and experimenting!

---

## ✨ About Me

- 💡 **Interested in:** strategic planning, remote sensing, masterplanning, and urban economics 
- ⚙️ **Tools I love:** geopandas, seaborn, DuckDB, QGIS, Grasshopper  

---

## 🚀 Featured Project

| Project                                                     | Description                                                                                    | Link                                             |
|-------------------------------------------------------------|------------------------------------------------------------------------------------------------|--------------------------------------------------|
| **Housing near train stations**    | Recently, the UK government announced to give "default yes" to housing applications near train stations. Is this a good idea? Where are the opportunity areas for further densification? I investigated the past planning applications database and employed K-Means clustering to answer the question.   | [Visit Site](https://boscochoi.github.io/Housing_Default_Yes_Project/qm_final_assm.html)    |
| **Hong Kong Planning Infohub**    | This app allows planning application documents to be accessed seamlessly using an interactive geospatial portal, bridging the existing gap between the Planning Deaprtment's SPP3 portal and Town Planning Board's document records.      | [View App](https://boscochoi.github.io/HK_Planning_Infohub/)    |
| **Airbnb in London**    | This group project explores whether Airbnb is out of control in London. It was prepared as a policy brief to the "Mayor" in a hypothetical scenario.      | [Visit Site](https://benjamintee.github.io/CASA_FSDS_Project/Group_Work.html)    |
| **Surface breakdown web app**    | This is a web app prototype developed in a one day hackathon with UCLSU and Homefolk. The app allows users to input a postcode and outputs breakdown of paved, non-paved areas and building footprint, aiming to support Homefolk's vision in indentifying areas for tiny infill houses.     | [View App](https://london-surface-area-breakdown.streamlit.app/)    |


